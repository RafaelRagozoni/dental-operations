...
from PyQt6.QtWidgets import QApplication, QWidget,QCheckBox, QPushButton, QLabel, QLineEdit, QComboBox
...
class DentalUI:
    ...
    def addTeethImages(self):
        ...
        for image_file in os.listdir(images_path):
            ...
            self.check_box_dentes[tooth_id] = self.addCheckBoxJanela((start_x+5, start_y+checkbox_y), self.janelaOperations)
            self.check_box_dentes[tooth_id].setStyleSheet("QCheckBox::indicator { width: 50px; height: 50px;}")
            start_x += 60
            contador+=1
            if contador == 16:
                start_x = 100
    ...
    def adicionaElementosJanelaPaciente(self):
        ...
        self.static_label_phone = self.addLabelJanela("Telefone", (20,370), self.janelaPessoas)
        self.static_label_last_seen = self.addLabelJanela("Data ultima consulta", (20,420), self.janelaPessoas)
        ...
        self.line_number_procedure = self.addLabelJanela("",(300,470), self.janelaPessoas)
    ...
    def adicionaElementosJanelaOperacoes(self):
        ...
        self.static_label_name_in_procedure = self.addLabelJanela("Nome: ", (20,90), self.janelaOperations)
        self.static_label_cpf_in_procedure = self.addLabelJanela("CPF: ", (20,140), self.janelaOperations)
        self.static_label_notes = self.addLabelJanela("notas: ", (20,190), self.janelaOperations)
        self.line_name_in_procedure = self.addLabelJanela("",(150,90), self.janelaOperations)
        self.line_cpf_in_procedure = self.addLabelJanela("",(150,140), self.janelaOperations)
    ...
    def adicionaElementosJanelaProcedimentos(self):
        ...
        for procedimento in procedimentos:
            self.check_box_procedimentos[procedimento] = self.addCheckBoxJanela((start_x, start_y), self.janela_procedimentos)
            self.check_box_procedimentos[procedimento].setStyleSheet("QCheckBox::indicator { width: 20px; height: 20px;}")
            self.addLabelJanela(procedimento, (start_x+30, start_y), self.janela_procedimentos)
            start_x += 400
            if start_x > 1200:
                start_x = 10
                start_y += 50
